/**
 * Instagram Direct — Meta messaging webhook (object: instagram).
 */
...
function mergeMeta(sessionId, igUserId) {
  try {
...
async function handleInboundMessage(igUserId, text) {
  ...
  try {
    ...
    console.error('[instagram] chat turn:', err.message);
    return null;
  }
  ...
  if (reply && meta.isMessengerConfigured()) {
    try {
      await meta.sendMessengerText(uid, reply);
    } catch (sendErr) {
      console.error('[instagram] send:', sendErr.message);
    }
  }
