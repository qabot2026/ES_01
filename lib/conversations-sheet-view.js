...
function defaultDateRange(daysBack = 4) {
...
function parseDateOpts(opts = {}) {
  ...
  let fromIso = fromIn && isoYyyyMmDdOk(fromIn) ? fromIn : null;
  let toIso = toIn && isoYyyyMmDdOk(toIn) ? toIn : null;
...
async function fetchConversationSheetPreview(opts = {}) {
  ...
  if (previewDateFilterActive) {
    ...
    const listCap = dateFilterMaxRows();
    const matchedChrono = [];
    const newestRing = [];
    let totalMatches = 0;
    for (let ri = 0; ri < dataRows.length; ri += 1) {
      ...
      if (allInRange) {
        newestRing.push(row);
        if (newestRing.length > viewerCap) newestRing.shift();
      } else if (matchedChrono.length < listCap) {
        matchedChrono.push(row);
      }
    }
    const rowsTruncated =
      totalMatches > (allInRange ? viewerCap : listCap) ||
      totalMatches > viewerCap;
    const newestFirst = allInRange
      ? newestRing.slice().reverse()
      : matchedChrono.slice().reverse();
    let sliceRows;
