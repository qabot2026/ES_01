...
function addProject({ id, name, welcomeEventName }) {
  ...
  const overrides = readOverrides_();
  if (!overrides[sitePreset]) {
    overrides[sitePreset] = cloneDefaultPresetForBot(botName);
