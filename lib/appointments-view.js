...
function mapSheetRow(row) {
  ...
  return {
    ...
    appointmentDate: String(row['App. Date'] || '').trim(),
    ...
    sessionId,
    transcriptUrl: link || (sessionId ? `/transcript.html?session=${encodeURIComponent(sessionId)}` : ''),
  };
}
...
function mapTranscriptDoc(doc) {
  ...
  return {
    ...
    conversationDate: String(doc.updatedAt || '').slice(0, 10),
    conversationTime: '',
    channel: String(meta.channel || 'Web').trim(),
    sessionId,
    transcriptUrl: sessionId
...
async function listFromSheet(opts) {
...
async function fetchAppointmentsList(opts) {
  const from =
    opts && typeof opts.from === 'string' ? opts.from.trim() : '';
  const to = opts && typeof opts.to === 'string' ? opts.to.trim() : '';
  let sheetRows = [];
  let source = 'transcript';

  if (sheets.isConfigured()) {
    try {
      sheetRows = await listFromSheet({ from, to });
  ...
  return {
    ...
    total: items.length,
    appointments: items,
    dateFilter: from || to ? { from: from || null, to: to || null } : null,
  };
}
