...
function normalizeUserQueryText(text) {
  ...
  // Form submit payloads: "[form:contact]\nstaff:contact\nName: ... | __GO_location__ | Upload | Good service"
  const formMatch = raw.match(/^\[form:([^\]\s]+)\]/i);
  if (formMatch) {
    const formId = formMatch[1];
    const human = titleCase(formId);

    const pipeParts = raw
      .split('|')
      .map((p) => String(p || '').trim())
      .filter((p) => !isInternalActionToken(p));

    // Keep only the most human-facing tail token (e.g. "Good service")
    const tail = pipeParts.length ? pipeParts[pipeParts.length - 1] : '';
    if (tail && !/^(name|code|mobile|email)\s*:/i.test(tail)) {
      return `${human} form filled (${tail})`;
    }
    return `${human} form filled`;
  }

  // Hide internal tokens if they somehow reach the transcript
