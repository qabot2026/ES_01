...
function documentForSheet(meta) {
  const m = meta || {};
  if (m.document_link) return String(m.document_link).trim();
  if (m.document_links) return String(m.document_links).trim();
  if (m.storage_folder) return String(m.storage_folder).trim();
  return scalar(m.document || m.upload);
}
...
async function runSheetSync(sessionId) {
  ...
  if (!chatTranscript.sessionHasUserEngagement(doc)) {
