...
function logDialogflowExchange(sessionId, userMessage, result, options) {
  ...
  const richMeta = richMetaFromDialogflowResult(result);
  const botText = botDisplayTextFromDialogflowResult(result);
  const hasBotContent =
    Boolean(richMeta) ||
    Boolean(botText && botText !== 'No response.');
  /* Form submit: widget shows ack and logs it; skip hidden Dialogflow reply only. */
  const skipBot =
    opts.skipTranscriptBot === true ||
    formSubmit ||
    (result.intentIsFallback && !hasBotContent);

  if (!skipBot) {
    if (botText) {
      appendTurn(sid, 'bot', botText, richMeta, noSchedule);
    } else if (richMeta) {
      appendTurn(sid, 'bot', '(Rich content)', richMeta, noSchedule);
    }
  }
