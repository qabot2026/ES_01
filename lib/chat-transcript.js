...
function appendTurn(sessionId, role, text, meta) {
...
function logDialogflowExchange(sessionId, userMessage, result) {
  ...
  if (userText) appendTurn(sid, 'user', userText);

  const parts = Array.isArray(result.replyParts) ? result.replyParts : [];
  if (parts.length) {
    parts.forEach((p) => {
      const t = p && p.text != null ? String(p.text).trim() : '';
      if (t) appendTurn(sid, 'bot', t);
    });
  } else {
    const reply = String(result.reply || '').trim();
    if (reply && reply !== 'No response.') appendTurn(sid, 'bot', reply);
  }

  if (result.intentIsFallback) {
    mergeSessionMeta(sid, { fallback: 'yes' });
  }
}
