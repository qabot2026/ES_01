...
function isObjectInBucket(gcsObject, existingGcsObjects) {
  const obj = String(gcsObject || '').trim();
  if (!obj) return false;
  if (isExternalCatalogObject(obj)) return true;
  return !!(existingGcsObjects && existingGcsObjects.has(obj));
}

function filterToExistingBucketFiles(files, existingGcsObjects) {
  return (files || []).filter((f) =>
    isObjectInBucket(f && f.gcs_object, existingGcsObjects)
  );
}
...
function supplementGroupsFromRecords(groups, records, existingGcsObjects) {
  (records || []).forEach((rec) => {
    if (!rec) return;
    const objectKey = String(rec.gcs_object || '').trim();
    if (!objectKey) return;
    if (!isObjectInBucket(objectKey, existingGcsObjects)) return;
...
async function listDocumentCatalog(opts) {
  ...
  const existingGcsObjects = buildExistingGcsObjectSet(files);
  ...
  supplementGroupsFromRecords(groups, transcriptRecords, existingGcsObjects);
  supplementGroupsFromRecords(groups, sheetRecords, existingGcsObjects);

  let folders = Array.from(groups.values());
  const seenObjectsGlobal = new Set();
  folders.forEach((f) => {
    f.files = filterToExistingBucketFiles(f.files, existingGcsObjects);
