...
function supplementGroupsFromRecords(groups, records) {
  (records || []).forEach((rec) => {
    if (!rec) return;
    const folder =
      rec.storage_folder ||
...
async function listDocumentCatalog(opts) {
  ...
  let [files] = await bucket.getFiles({ prefix, autoPaginate: true });
  ...
  supplementGroupsFromRecords(groups, transcriptRecords);
  supplementGroupsFromRecords(groups, sheetRecords);
