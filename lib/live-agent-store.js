...
function filterInboxForAgentDepartment(list, agentEmail) {
  ...
  return list.filter((c) => {
    ...
    return agentCanAccessDepartment(me, c.departmentId);
  });
}
...
function isDialogflowBlockedForSession(sessionId) {
  ...
  if (s) {
    ...
    return visitorHumanChatActive(serializeConversation(s.sessionId, s));
  }
  try {
    const doc = chatTranscript.getSessionDoc(sid);
    const meta = doc.meta && typeof doc.meta === 'object' ? doc.meta : {};
    if (meta.liveAgentBotHandoff === true && meta.liveAgentActive !== true) {
      return false;
    }
...
function listMessages({
  ...
  return msgs.map((m) => {
    ...
    if (normalizeMessageRole(m) === 'system') {
      return {
        ...base,
