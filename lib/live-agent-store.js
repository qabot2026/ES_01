...
/** Visitor poll: store messages + transcript agent turns (fallback). */
function listMessagesForVisitor(conversationId, opts) {
  ...
  try {
    const doc = chatTranscript.getSessionDoc(conversationId);
    (doc.turns || []).forEach((t) => {
      if (!t || t.role !== 'agent') return;
      const text = trim(t.text);
      if (!text) return;
      if (t.id && seen.has(t.id)) return;
      const sm = serializeMessage({
        id: t.id || randomUUID(),
        role: 'agent',
        text,
        createdAt: t.at || t.createdAt,
      });
      msgs.push(sm);
      if (sm.id) seen.add(sm.id);
    });
    msgs.sort((a, b) =>
      String(a.createdAt || '').localeCompare(String(b.createdAt || ''))
    );
    msgs = msgs.slice(-Math.min(tail, 100));
  } catch (e) {
    console.warn('[live-agent] transcript merge:', e.message);
  }
  return msgs;
}
...
function closeConversation({ conversationId, agentEmail, closedBy }) {
  ...
  try {
    chatTranscript.mergeSessionMeta(s.sessionId, { liveAgentActive: false });
  } catch {
    /* ignore */
  }
  saveStore();
