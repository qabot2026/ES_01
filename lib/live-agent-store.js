...
const GCS_PULL_MIN_MS = Math.max(
  120,
  Number(process.env.LIVE_AGENT_GCS_PULL_MS) || 150
);
...
function enrichPayloadFromSignals(sessionId, payload) {
  ...
  const vt = typingFromSignalText(sig.visitorTyping, sig.visitorTypingAt);
  if (vt) payload.visitorTyping = vt;
  const at = typingFromSignalText(sig.agentTyping, sig.agentTypingAt);
  if (at) payload.agentTyping = at;
...
function serializeConversation(id, s) {
  ...
  return {
    ...
    visitorTypingText: activeTypingText(s.visitorTypingText, s.visitorTypingAt),
...
async function updateVisitorTyping({ conversationId, text, active }) {
  const id = safeId(conversationId);
  const s = getSession(id);
  if (!s || s.status === 'closed') return null;
  const on = active !== false && trim(text).length > 0;
  s.visitorTypingText = on ? String(text).slice(0, 400) : '';
  s.visitorTypingAt = on ? nowIso() : '';
...
function buildVisitorSyncPayload(clientSessionId) {
  ...
  const visitorTyping = conversation
    ? activeTypingText(conversation.visitorTypingText, conversation.visitorTypingAt)
    : '';
  const agentTyping = conversation
    ? activeTypingText(conversation.agentTypingText, conversation.agentTypingAt)
    : '';
  return {
    ok: true,
    revision,
...
function postVisitorMessage({ conversationId, text }) {
  ...
  let s = getSession(id);
  if (!s) {
    throw new Error('No active human chat');
  }
...
function updateConversationMode({ conversationId, aiEnabled, humanMode }) {
  ...
  const botOn =
    hm === 'ai' || (typeof s.aiEnabled === 'boolean' && s.aiEnabled === true);
  try {
    if (botOn) {
      chatTranscript.mergeSessionMeta(s.sessionId, {
        liveAgentActive: false,
      ...
      appendMessage(s, {
        role: 'system',
        text: 'The assistant is replying again.',
        bumpUnread: { visitor: 1, agent: 0 },
      });
    } else if (hm === 'human') {
      chatTranscript.mergeSessionMeta(s.sessionId, {
