...
function getSession(sessionId) {
  loadStore();
  return store.sessions[safeId(sessionId)] || null;
}
...
function serializeMessage(m) {
  const role = trim(m.role) || 'visitor';
  return {
    id: m.id,
    role,
    text: m.text || '',
...
function listInbox({ status, agentEmail, limit }) {
  ...
  let list = Object.values(store.sessions).map((s) =>
    serializeConversation(s.sessionId, s)
  );
...
function acceptConversation({ conversationId, agentEmail }) {
  ...
  s.updatedAt = t;
  s.unreadForAgent = 0;
  const display = me.split('@')[0];
  appendMessage(s, {
    role: 'system',
    text: `${display} joined the chat.`,
    senderEmail: me,
...
function listMessages({ conversationId, sinceIso, limit, markReadFor }) {
  const s = getSession(conversationId);
  if (!s) return [];
  const sinceMs = sinceIso ? Date.parse(sinceIso) : 0;
  let msgs = (s.messages || []).filter((m) => {
...
function postAgentMessage({ conversationId, text, agentEmail, agentName }) {
  ...
  const msg = appendMessage(s, {
    ...
    text,
    senderEmail: me,
    senderDisplayName: trim(agentName) || me.split('@')[0],
    bumpUnread: { visitor: 1, agent: 0 },
  });
  saveStore();
