...
/**
 * Find sheet row by Session ID column (K / index 10).
...
async function fetchSheetRowBySessionId(sessionId) {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      const row = rows[i];
      if (String((row && row[10]) || '').trim() !== sid) continue;
      const columns = {};
...
/**
 * Mobile numbers already in the sheet (column E), excluding one row when updating.
...
async function loadDocumentEnrichmentByFolder() {
  ...
  try {
    ...
    const rows = res.data.values || [];
    rows.forEach((row) => {
      const name = String((row && row[3]) || '').trim();
      const mobile = String((row && row[4]) || '').trim();
      const sessionId = String((row && row[10]) || '').trim();
      const document = String((row && row[19]) || '').trim();
      const date = String((row && row[1]) || '').trim();
      const time = String((row && row[2]) || '').trim();
