...
function parseQueryResult(response, languageCode = 'en') {
  ...
  const { textParts, replyParts, chips, chipHeading, infoCards, downloads, dropdowns } =
  ...
  if (!reply && dropdowns.length) {
    reply = dropdowns.map((d) => d.message).filter(Boolean).join('\n').trim();
  }
