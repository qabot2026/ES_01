...
function parseQueryResult(response, languageCode = 'en') {
  ...
  let { textParts, chips } = parseFulfillmentMessages(
    result.fulfillmentMessages
  );
  const fulfillmentText = (result.fulfillmentText || '').trim();
  if (fulfillmentText) {
    const key = fulfillmentText.toLowerCase();
    const already = textParts.some((t) => t.toLowerCase() === key);
    if (!already) textParts.push(fulfillmentText);
  }
  const reply =
    textParts.join('\n').trim() ||
    (chips.length ? '' : 'No response.');
  return {
    reply,
    chips,
