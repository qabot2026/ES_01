...
function mountLiveAgentRoutes(app) {
  ...
  router.get('/inbox', requireAgentSession, (req, res) => {
    setNoCache(res);
    try {
  ...
  router.get('/conversations/:id/messages', requireAgentSession, (req, res) => {
    setNoCache(res);
    try {
  ...
  router.post('/conversations/:id/messages', requireAgentSession, (req, res) => {
    setNoCache(res);
  ...
  router.post('/conversations/:id/close', requireAgentSession, (req, res) => {
    setNoCache(res);
    try {
  ...
  router.post('/conversations/:id/mode', requireAgentSession, (req, res) => {
    setNoCache(res);
  ...
  publicRouter.post('/request', (req, res) => {
    setPublicCors(res);
    setNoCache(res);
  ...
  publicRouter.get('/status', (req, res) => {
    ...
    const agentDisplayName = conversation
      ...
      : '';
    const humanMode = conversation ? conversation.humanMode : 'ai';
    const aiEnabled =
  ...
  publicRouter.get('/messages', (req, res) => {
    ...
    try {
      ...
      const messages = store.listMessages({
        ...
        sinceIso: trim(req.query.since) || undefined,
        sinceId: trim(req.query.sinceId) || undefined,
        tail: Number.isFinite(tail) && tail > 0 ? tail : undefined,
        markReadFor: trim(req.query.markRead) === '1' ? 'visitor' : undefined,
        audience: 'visitor',
      });
  ...
  publicRouter.post('/visitor-message', (req, res) => {
    setPublicCors(res);
    setNoCache(res);
    const clientSessionId = trim(
      (req.body && req.body.clientSessionId) || (req.body && req.body.sessionId)
    ...
    try {
      ...
      res.json({ ok: true, ...result, deduped: false });
    } catch (err) {
      jsonError(res, 400, err.message || 'Send failed');
    }
  });
