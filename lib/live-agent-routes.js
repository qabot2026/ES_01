...
async function buildVisitorSyncAfterSignals(sessionId, clientRev) {
  ...
  if (remoteRev > clientRev) {
    await store.syncPull({ force: true });
  } else {
    await store.syncPull({ maxAgeMs: 250 });
  }
  return store.buildVisitorSyncPayload(sessionId);
}

async function longPollUntilChange(sessionId, clientRev, waitMs) {
  const deadline =
    Date.now() + Math.min(Math.max(Number(waitMs) || 20000, 400), 28000);
  let seenTyping = '';
  let seenAgentTyping = '';
  while (Date.now() < deadline) {
    await signals.pullSignals({ maxAgeMs: 100 });
    const sig = signals.getSessionSignal(sessionId);
    const remoteRev = sig ? Number(sig.revision) || 0 : 0;
    ...
    if (
      ...
      return buildVisitorSyncAfterSignals(sessionId, clientRev);
    }
    seenTyping = vt;
    seenAgentTyping = at;
    await sleep(200);
  }
  const payload = await buildVisitorSyncAfterSignals(sessionId, clientRev);
  if (
...
function mountLiveAgentRoutes(app) {
  ...
  router.post('/conversations/:id/typing', requireAgentSession, async (req, res) => {
    ...
    try {
      await store.syncPull({ maxAgeMs: 200 });
  ...
  publicRouter.get('/sync', async (req, res) => {
    ...
    try {
      const payload =
        waitMs > 0
          ? await longPollUntilChange(clientSessionId, clientRev, waitMs)
          : await buildVisitorSyncAfterSignals(clientSessionId, clientRev);
      res.json(payload);
    } catch (err) {
