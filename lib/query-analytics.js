...
function eventsForOutcomeTable(events, outcome) {
  ...
  if (outcome === 'bot') {
    ...
    return events.filter(
      (e) => e.outcome === 'bot' && !fallbackBuckets.has(eventMinuteBucketKey(e))
    );
  }
...
function getQueryAnalytics(opts) {
  ...
  return {
    ...
    answeredQueries: aggregateQueriesForOutcome(inRange, 'bot').slice(0, limit),
    unansweredQueries: aggregateQueriesForOutcome(inRange, 'fallback').slice(
      0,
      limit
    ),
    sources: {
