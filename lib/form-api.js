...
/** On today, drop slots that already started (local time). */
function filterPastSlotsToday(slots24, dateIso, hidePastTimesToday) {
  if (!hidePastTimesToday || dateIso !== todayLocalIso()) return slots24;
...
function appointmentSlots(formIdOrScope, _doctorId, date) {
  ...
  if (resolved.closed || !allSlots24.length) {
    return {
      date: day,
      formId,
      closed: true,
...
function appointmentMonthSummary(formIdOrScope, year, month) {
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    if (r.closed || r.todayHidden || r.outsideWindow) {
      closedDates.push(iso);
    } else if (
      r.allFull ||
