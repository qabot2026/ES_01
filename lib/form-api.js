...
function appointmentSlots(scope, doctorId, date, options = {}) {
  ...
  const scopeKey = scope === 'doctor' ? 'doctor' : 'general';
  if (!/^\d{4}-\d{2}-\d{2}$/.test(day)) {
    return { error: 'invalid_date', bookedTimes: [], fullyBookedDates: [] };
  }
  if (day < todayLocalIso()) {
    return {
      error: 'past_date',
      date: day,
      scope: scopeKey,
  ...
  const resolved = appointmentSchedule.resolveForDate(scopeKey, day);
  const allSlots = appointmentSchedule.buildAllSlots(resolved);

  if (resolved.closed || !allSlots.length) {
    return {
      date: day,
      scope: scopeKey,
      doctorId: doctorId || '',
  ...
  const booked24 = bookedTimes.map((t) => to24h(t)).filter(Boolean);
  const availableTimes = allSlots.filter((t) => booked24.indexOf(t) < 0);
  const allSlots12 = allSlots.map((t) => to12h(t));
  const availableTimes12 = availableTimes.map((t) => to12h(t));
  const bookedTimes12 = booked24.map((t) => to12h(t));
  ...
  return {
    ...
    periods: resolved.periods,
    fullyBookedDates,
    bookedTimes: bookedTimes12,
    availableTimes: availableTimes12,
    allSlots: allSlots12,
    /** 24h values for internal matching (optional). */
    allSlots24: allSlots,
  };
}
