...
function parseCardCarousel(plain) {
  ...
  const cards = rawCards
    .map((card) => {
      ...
      const ctaLabel = String(card.ctaLabel || card.cta_label || 'View').trim();
      const ctaMessage = String(
        card.ctaValue ?? card.cta_value ?? card.message ?? card.postback ?? ctaLabel
      ).trim();
      ...
      return {
        ...
        ctaMessage: ctaMessage || ctaLabel || 'View',
      };
    })
    .filter(Boolean);
...
function parseInlineSelect(plain) {
  ...
  return {
    message: String(plain.message || plain.title || plain.prompt || '').trim(),
    placeholder: String(plain.placeholder || 'Choose…').trim(),
