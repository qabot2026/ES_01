...
function parseOpenForm(plain) {
  ...
  const nextFormIds = Array.isArray(plain.next_form_ids || plain.nextFormIds)
    ? (plain.next_form_ids || plain.nextFormIds).map((id) => String(id || '').trim()).filter(Boolean)
    : [];

  return {
    formId,
    message: String(plain.message || plain.title || plain.prompt || '').trim(),
    nextFormId: String(plain.next_form_id || plain.nextFormId || '').trim(),
    nextFormIds,
    prefill:
      plain.prefill ||
      plain.field_values ||
